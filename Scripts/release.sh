...
APP_NAME="${APP_NAME:-Rhythm}"
APP_BUNDLE="${APP_BUNDLE:-${APP_NAME}.app}"
PROJECT_NAME="${PROJECT_NAME:-Rhythm}"
PROJECT_PATH="${PROJECT_PATH:-${PROJECT_NAME}.xcodeproj}"
KEYCHAIN_PROFILE="${KEYCHAIN_PROFILE:-}"
VERSION="${VERSION:-}"
BUILD_NUMBER="${BUILD_NUMBER:-}"
SCHEME="${SCHEME:-Rhythm}"
CONFIGURATION="${CONFIGURATION:-Release}"
DESTINATION="${DESTINATION:-platform=macOS}"
BUILD_CHECK_SIGNING_ALLOWED="${BUILD_CHECK_SIGNING_ALLOWED:-NO}"
PROJECT_FILE="${PROJECT_FILE:-${PROJECT_NAME}.xcodeproj/project.pbxproj}"
...
Commands:
  ...
  push-tags   Push the release tag to origin
  release     Create a GitHub release (no assets)
  upload      Upload the release asset to GitHub
  help        Show this help

Environment:
  APP_NAME          App name (default: Rhythm)
  APP_BUNDLE        App bundle path (default: ${APP_NAME}.app)
  PROJECT_NAME      Xcode project name (default: Rhythm)
  PROJECT_PATH      Xcode project path (default: ${PROJECT_NAME}.xcodeproj)
  ...
  SCHEME            Xcode scheme for build check (default: Rhythm)
  ...
  BUILD_CHECK_SIGNING_ALLOWED Whether release check signs the build (default: NO)
  PROJECT_FILE      Xcode project file (default: ${PROJECT_NAME}.xcodeproj/project.pbxproj)
...
resolve_bundle_id() {
  ...
  while IFS= read -r line; do
    if [[ "${line}" == *"PRODUCT_BUNDLE_IDENTIFIER ="* && "${line}" != *"rhythm-server"* ]]; then
...
list_profiles() {
  local profiles_dir
  local found_dir="0"
  local profile tmp_plist name uuid team app_id
  local profiles_dirs=(
...
build_check() {
  ...
  xcodebuild -quiet \
    ...
    -configuration "${CONFIGURATION}" \
    -destination "${DESTINATION}" \
    CODE_SIGNING_ALLOWED="${BUILD_CHECK_SIGNING_ALLOWED}" \
    build
  resolve_app_bundle
}
